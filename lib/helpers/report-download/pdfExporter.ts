...
export async function exportPdf({
  ...
  try {
    ...
    const pdfFontsClient = pdfFonts as unknown as {
      vfs: Record<string, string>;
    };
          ...
          table: {
            ...
            body: [
              [
                ...
                {
                  text: `${section.previousYear}`,
                  colSpan: 2,
                  style: "monthlyHeader",
                },
                { text: "", style: "monthlyHeader" },
                {
                  text: `${section.currentYear}`,
                  colSpan: 2,
                  style: "monthlyHeader",
                },
                { text: "", style: "monthlyHeader" },
              ...
              ...section.rows.map((row) => [
                ...
                {
                  text: row.label,
                  style: row.isTotalRow ? "monthlyTotalLeft" : "monthlyLeft",
                },
                {
    ...
    const tableWidths = [
      "*",
      ...processedData.headers.slice(1).map(() => "auto"),
    ];
          ...
          table: {
            ...
            body: [
              processedData.headers.map((header, headerIndex) => ({
                text:
                  headerIndex === 0 ? String(header) : `${String(header)} [t]`,
                color: "#ffffff",
                bold: true,
                fontSize: 9,
              ...
              ...processedData.rows.map((row, rowIndex) =>
                row.map((cell, cellIndex) => ({
                  text:
                    cellIndex === 0 ? String(cell) : formatNumber(Number(cell)),
                  fontSize: 9,
    ...
    pdfMakeClient
      ...
      .download(getFilename("pdf", startDate, endDate));
