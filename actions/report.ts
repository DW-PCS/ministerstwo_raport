...
export async function fetchPortsAction(): Promise<AppClientsTypes[]> {
  ...
  return data.map((item: { Id: number; Enabled: boolean; Name: string; City: string; OrgName: string }) => ({
    id: item.Id,
...
export async function fetchReportDataAction(
  ...
  return data.map((item: { Port: string; Kod?: string; Grupa?: string; Ilosc: number }) => ({
    port: item.Port,
